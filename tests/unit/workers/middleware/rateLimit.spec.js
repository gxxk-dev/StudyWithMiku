...
describe('workers/middleware/rateLimit', () => {
  describe('rateLimit', () => {
    ...
    it('窗口过期后重置计数', async () => {
      ...
      // 第 1 次通过
      const c1 = createMockContext(ip)
      await middleware(c1, vi.fn())

      // 第 2 次被限制
      const c2 = createMockContext(ip)
    ...
    it('不同 IP 独立计数', async () => {
      ...
      // IP1 第 1 次通过
      const c1 = createMockContext(ip1)
      await middleware(c1, vi.fn())

      // IP1 第 2 次被限制
      const c1b = createMockContext(ip1)
      await middleware(c1b, vi.fn())
      expect(c1b.json).toHaveBeenCalled()

      // IP2 第 1 次仍然通过
      const c2 = createMockContext(ip2)
      const next2 = vi.fn()
    ...
    it('不同 keyPrefix 独立计数', async () => {
      ...
      // prefixA 第 2 次被限制
      const c1b = createMockContext(ip)
  ...
  describe('getClientIP', () => {
    it('优先使用 CF-Connecting-IP', async () => {
      ...
      await middleware(c, vi.fn())
      // 验证使用了 CF-Connecting-IP（通过 header 调用）
  ...
  describe('cleanupExpiredEntries', () => {
    it('概率触发清理（Math.random < 0.01）', async () => {
      const ip = uniqueIp()
      const middleware = rateLimit({ windowMs: 60000, max: 100, keyPrefix: 'cleanup' })

      // 强制触发清理
      const originalRandom = Math.random
      Math.random = () => 0.005 // < 0.01，触发清理
