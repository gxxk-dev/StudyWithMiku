 ...
 * @param {string[]} warnings - 警告信息数组（用于收集警告）
 * @returns {Object|null} { platform, id } 或 null（无效）
 */
function validatePlaylist(value, warnings) {
  const pattern = /^(netease|tencent|spotify):[a-zA-Z0-9_-]+$/
  const decoded = decodeURIComponent(value)

  if (!pattern.test(decoded)) {
    warnings.push(`playlist: 无效格式 "${value}"，格式应为 platform:id`)
    return null
  }
 ...
 */
function validateNumber(name, value, min, max, warnings) {
  const num = parseInt(value, 10)
  if (isNaN(num) || num < min || num > max) {
    warnings.push(`${name}: 无效值 "${value}"，应为 ${min}-${max} 之间的整数`)
    return null
  }
  return num
}
 ...
 *
 * @param {string} searchString - URL 的 search 部分（可以带或不带 ?）
 * @returns {Object} { config, warnings }
 */
export function parseUrlString(searchString) {
  const warnings = []
  ...
  if (playlist) {
    const parsed = validatePlaylist(playlist, warnings)
    if (parsed) {
  ...
  if (focus) {
    const parsed = validateNumber('focus', focus, 1, 120, warnings)
    if (parsed !== null) {
  ...
  if (short) {
    const parsed = validateNumber('short', short, 0, 60, warnings)
    if (parsed !== null) {
  ...
  if (long) {
    const parsed = validateNumber('long', long, 0, 60, warnings)
    if (parsed !== null) {
  ...
  if (interval) {
    const parsed = validateNumber('interval', interval, 1, 10, warnings)
    if (parsed !== null) {
...
export function useUrlParams() {
  ...
  // 解析 URL 参数
  const { config, warnings } = parseUrlString(window.location.search)
  const urlConfig = ref(config)
